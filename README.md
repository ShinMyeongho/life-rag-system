...
> 비개발자가 직접 만든 개인 자기관리 웹 애플리케이션입니다.  
> 코딩을 전문으로 배운 적 없이, AI 도구를 활용해 기획부터 구현까지 혼자 완성했습니다.
...
개발 경력이 없어서 시행착오가 많았지만, 그 과정에서 데이터 설계·AI 연동·테스트 작성까지 직접 경험했습니다.
...
- **DB 설계를 몰랐음** → 테이블 구조를 여러 번 갈아엎으며 upsert 패턴을 익혔습니다
- **RAG가 뭔지 몰랐음** → 단순 AI 호출과 다르게 문서를 검색해서 근거를 주는 구조를 직접 구현했습니다
- **테스트를 안 짜다가 버그가 쌓임** → 뒤늦게 pytest로 127개 단위 테스트를 작성했습니다
- **배포가 막막했음** → Docker로 환경을 통일하고, `.env`·`user_config.py`를 분리해 개인정보를 관리했습니다
